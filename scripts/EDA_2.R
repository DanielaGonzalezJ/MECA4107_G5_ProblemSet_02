...
setwd("/Users/dngonzalez/Documents/1. Universidad/Maestría/BDML_2024/repos/MECA4107_G5_ProblemSet_02")
train_hogares <- read.csv("sources/train_hogares.csv")
train_personas <- read.csv("sources/train_personas.csv")
test_hogares <- read.csv("sources/test_hogares.csv")
test_personas <- read.csv("sources/test_personas.csv")
...
summary(train_personas$P6500)  # Get summary statistics (mean, median, etc.) for income in 'train' data

ggplot(train, aes(x = jefe_edad)) +
  geom_histogram(bins = 10, color = "blue", fill = "lightblue") +
  labs(title = "Distribución de edad del jefe de hogar", x = "Edad", y = "Frequency")


ggplot(train_personas, aes(x = P6500)) +
  geom_histogram(bins = 15, color = "blue", fill = "lightblue") +
  xlim(0,10000000) + 
  labs(title = "Distribución del ingreso de los individuos", x = "Ingreso", y = "Frequency")


ggplot(train, aes(x = factor(Pobre), y = horas_trab_por_persona)) +
  geom_boxplot() +
  labs(title = "Distribución de horas trabajadas", 
       x = "Status Pobreza", y = "Work Hours")
ggsave(filename = "work_hours_boxplot.png", width = 8, height = 6)


ggplot(train, aes(x = factor(jefe_edad), y = horas_trab_por_persona)) +
  geom_boxplot() +
  labs(title = "Distribución de edad del jefe del hogar vs. Status de Pobreza", 
       x = "Horas trabajadas", y = "Edad del jefe del hogar")
ggsave(filename = "age_hours_boxplot.png", width = 8, height = 6)
